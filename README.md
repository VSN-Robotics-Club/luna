# luna
# 🤖 Luna: The AI-Powered Raspberry Pi Robot by VSN Robotics Club


```markdown
# 🌌 Luna: The AI-Powered Raspberry Pi Robot 🤖 by VSN Robotics Club

Meet **Luna**, the intelligent and interactive AI assistant developed by the VSN Robotics Club! Luna is designed to communicate, answer questions, and recognize objects in real-time. Powered by YOLOv8 for object detection and custom-trained GPT for conversational intelligence, Luna brings AI to life!

---

## 🚀 Key Features

- 🌐 **Conversational AI**: Ask Luna anything! Equipped with a custom-trained GPT model, Luna responds to your questions with depth and intelligence.
- 🎙️ **Voice Command**: Go hands-free! Command Luna with your voice to perform tasks effortlessly.
- 🕵️ **Object Detection**: Luna identifies objects with precision using YOLOv8, displaying names of recognized objects in real-time.
- 🔋 **Efficient Power Usage**: Battery-powered with backup support for uninterrupted performance.
- 🖥️ **Visual Display**: Luna provides dynamic feedback on its integrated screen, adding a visual element to every interaction.

---

## 🛠️ Installation Guide

1. **Clone the Repository**:
   ```bash
   git clone https://github.com/vsnroboticsclub/luna-ai.git
   cd luna-ai
   ```

2. **Install Dependencies**:
   ```bash
   sudo apt update && sudo apt install -y python3 python3-pip
   pip3 install -r requirements.txt
   ```

3. **Run the Luna AI Interface**:
   ```bash
   python3 luna_main.py
   ```

4. **Enable Voice & Object Detection**: Follow the steps in `docs/SETUP.md` for advanced features.

---

## 👥 Meet the Amazing Team Behind Luna 🌟

These fantastic contributors made Luna come to life! Click on their names to learn more about them. 🧑‍💻

| Name | Role | Link |
|------|------|------|
| [**Evan Lama**](https://www.facebook.com/profile.php?id=100069677232471) | Lead Developer 🛠️ | [Profile](https://www.facebook.com/profile.php?id=100069677232471) |
| [**Kishwor Dulal / Unknowne Hart**](https://www.facebook.com/kishwor.dulal.19) | AI Specialist 🧠 | [Profile](https://www.facebook.com/kishwor.dulal.19) |
| [**Nishant Kumar Bhandari**](https://www.facebook.com/nishantkrbhandari) | Raspberry Pi Integration Expert 📡 | [Profile](https://www.facebook.com/nishantkrbhandari) |
| [**Yugal Limbu**](https://www.facebook.com/messages/t/100051153849974) | Developer 💻 | [Profile](https://www.facebook.com/messages/t/100051153849974) |
| [**Bishal Laxmi Gurung**](https://www.facebook.com/bishallaxmigurung) | Project Head 🏆 | [Profile](https://www.facebook.com/bishallaxmigurung) |
| [**Sawol Singh**](https://www.facebook.com/sawol.singh) | 3D Designer 🎨 | [Profile](https://www.facebook.com/sawol.singh) |
| [**Aayush Neupane**](https://www.facebook.com/aayush.neupane.5099) | Security Handler 🔐 | [Profile](https://www.facebook.com/aayush.neupane.5099) |
| [**Anjali Rajya Laxmi Shahi**](https://www.facebook.com/profile.php?id=100090668433570) | Event Manager 🎉 | [Profile](https://www.facebook.com/profile.php?id=100090668433570) |
| [**Katrina Bk**](https://www.facebook.com/profile.php?id=61553518185297) | Event Manager 🎈 | [Profile](https://www.facebook.com/profile.php?id=61553518185297) |
| [**Bijay Shankar**](https://www.facebook.com/bijay.shankar.56884) | Physical Body & Electrical Expert ⚡ | [Profile](https://www.facebook.com/bijay.shankar.56884) |

We welcome contributions and feedback! 🌱 Reach out to any of the team members or open an issue to report bugs or suggest new features.

---

## ✨ Future Enhancements

- **Cool Visual Effects**: Introducing animations for object detection sequences to make Luna’s actions visually captivating.
- **3D Avatar**: A 3D animated version of Luna’s avatar to interact on screen.
- **Extended Object Recognition**: Expand YOLOv8’s object detection library for enhanced environment awareness.
- **Multi-User Support**: Enable simultaneous user interaction with Luna.

Stay tuned as Luna evolves, bringing more functionality and excitement! ✨
